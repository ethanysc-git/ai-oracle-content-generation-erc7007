...
import "./ERC7007Opml.sol";

// this contract is for ai.ora.io website
contract Prompt is AIOracleCallbackReceiver, ERC7007Opml {
    ...
    constructor(address _aiOracleAddress) AIOracleCallbackReceiver() {
        owner = msg.sender;
        aiOracle = IAIOracle(_aiOracleAddress);
        callbackGasLimit[50] = 1_000_000; // SD 500k
        callbackGasLimit[11] = 5_000_000; // llama
        callbackGasLimit[9] = 5_000_000; // grok
    }

    function setAiOracle(address _aiOracleAddress) external override onlyOwner {
        aiOracle = IAIOracle(_aiOracleAddress);
    }
